        ...
        <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-200 p-6 flex flex-col h-full">
                ...
                <div className="flex justify-between items-start mb-4">
                    ...
                    {showActions && auth.user?.roles.includes('recruiter') && (
                        <div className="flex items-center space-x-2 flex-shrink-0">
                            <Link to={`/jobs/${job.id}/applications`} title="View Applications" className="p-2 text-green-600 hover:bg-green-100 rounded-full transition-colors">
                                <Users className="h-5 w-5" />
                            </Link>
                            <button onClick={() => onEdit?.(job)} title="Edit Job" className="p-2 text-blue-600 hover:bg-blue-100 rounded-full transition-colors">
                                <Edit className="h-5 w-5" />
                            </button>
                            <button onClick={() => onDelete?.(job.id)} title="Delete Job" className="p-2 text-red-600 hover:bg-red-100 rounded-full transition-colors">
            ...
            <div className="mt-auto">
                ...
                {!showActions && auth.isAuthenticated && (
                    <button onClick={handleApply} disabled={isApplying} className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-semibold">
